...
public sealed class ReportCatalogService
{
    public IReadOnlyList<DependenciaCategoria> GetCategories()
    {
        ...
        static void Add(DependenciaCategoria category, string title, ZendeskDependencia dependencia,
            ...
            int idArea = 0, int idForm = 0, string instructions = "")
        {
            category.Reportes.Add(new ScreenReport(title, instructions, true, true, true, false,
                dependencia, idSeccion, idRequest, idGroup, idPriority, idStatus, idArea, idForm, GuessIcon(title)));
        }

        var obras = AddCategory(1, "Obras Publicas");
        Add(obras, "Atencion de bacheo", ZendeskDependencia.ObrasPublicas, 0, 18, 4, 1, 1, 5, 1);
        Add(obras, "Atencion a recoleccion de escombro", ZendeskDependencia.ObrasPublicas, 0, 19, 4, 1, 1, 5, 1);

        var servicios = AddCategory(2, "Servicios Publicos");
        Add(servicios, "Atencion al mantenimiento de alumbrado publico", ZendeskDependencia.ServiciosPublicos, 0, 1, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a caida de arboles", ZendeskDependencia.ServiciosPublicos, 0, 2, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a recoleccion de residuos solidos", ZendeskDependencia.ServiciosPublicos, 0, 3, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a camion de basura sin pasar", ZendeskDependencia.ServiciosPublicos, 0, 4, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion al mantenimiento de cesped en espacios publicos", ZendeskDependencia.ServiciosPublicos, 0, 5, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a recoleccion de cadaveres de animales en via publica", ZendeskDependencia.ServiciosPublicos, 0, 6, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a recoleccion de residuos verdes", ZendeskDependencia.ServiciosPublicos, 0, 7, 1, 1, 1, 2, 1);
        Add(servicios, "Atencion a mantenimiento de juegos en mal estado", ZendeskDependencia.ServiciosPublicos, 0, 8, 1, 1, 1, 2, 1);

        var opdapas = AddCategory(3, "OPDAPAS");
        Add(opdapas, "Atencion inmediata a fuga de agua", ZendeskDependencia.Opdapas, 0, 12, 3, 1, 1, 4, 1);
        Add(opdapas, "Atencion de descarga de aguas residuales", ZendeskDependencia.Opdapas, 0, 13, 3, 1, 1, 4, 1);
        Add(opdapas, "Atencion en limpieza y/o reparacion de alcantarilla", ZendeskDependencia.Opdapas, 0, 14, 3, 1, 1, 4, 1);
        Add(opdapas, "Atencion a servicio de desazolve", ZendeskDependencia.Opdapas, 0, 15, 3, 1, 1, 4, 1);
        Add(opdapas, "Atencion a coladera sin tapa", ZendeskDependencia.Opdapas, 0, 17, 3, 1, 1, 4, 1);
        Add(opdapas, "Atencion a inundaciones", ZendeskDependencia.Opdapas, 0, 16, 3, 1, 1, 4, 1);

        var ambiente = AddCategory(4, "Medio Ambiente");
        Add(ambiente, "Atencion a control canino", ZendeskDependencia.MedioAmbiente, 0, 20, 5, 1, 1, 6, 1);
        Add(ambiente, "Atencion a podas y derribos", ZendeskDependencia.MedioAmbiente, 0, 21, 5, 1, 1, 6, 1);

        var proteccion = AddCategory(5, "Proteccion Civil");
        Add(proteccion, "Atencion inmediata a fuga de Gas L.P y Natural", ZendeskDependencia.ProteccionCivil, 0, 41, 8, 1, 1, 9, 1);
        Add(proteccion, "Atencion a incendio", ZendeskDependencia.ProteccionCivil, 0, 42, 8, 1, 1, 9, 1);
        Add(proteccion, "Atencion a servicio de ambulancia", ZendeskDependencia.ProteccionCivil, 0, 43, 8, 1, 1, 9, 1);
        Add(proteccion, "Atencion a enjambre de abejas", ZendeskDependencia.ProteccionCivil, 0, 44, 8, 1, 1, 9, 1);
        Add(proteccion, "Atencion / supervision de danos estructurales", ZendeskDependencia.ProteccionCivil, 0, 45, 8, 1, 1, 9, 1);

        var seguridad = AddCategory(6, "Seguridad Publica");
        Add(seguridad, "Atencion a semaforo sin funcionar", ZendeskDependencia.SeguridadPublica, 0, 37, 2, 1, 1, 3, 1);
        Add(seguridad, "Atencion a vehiculos en via publica con huella de desmantelamiento", ZendeskDependencia.SeguridadPublica, 0, 39, 2, 1, 1, 3, 1);
        Add(seguridad, "Atencion a senalamientos de transito en mal estado", ZendeskDependencia.SeguridadPublica, 0, 38, 2, 1, 1, 3, 1);
        Add(seguridad, "Atencion y/o dictamen de reductores de velocidad", ZendeskDependencia.SeguridadPublica, 0, 40, 2, 1, 1, 3, 1,
            "Requerimientos de la solicitud:\n1. Alto flujo vehicular y exceso de velocidad.\n2. Presencia de escuelas, areas de recreacion u hospitales con flujo peatonal alto.\n3. La colocacion del dispositivo debera atender una necesidad de 24 horas al dia.");
        Add(seguridad, "Atencion a apoyo vial", ZendeskDependencia.SeguridadPublica, 0, 34, 2, 1, 1, 3, 1);

        var urbano = AddCategory(7, "Desarrollo Urbano");
        Add(urbano, "Atencion a obra sin permiso de construccion", ZendeskDependencia.DesarrolloUrbano, 0, 24, 6, 1, 1, 7, 1);
        ...
        Add(gobernacion, "Atencion a verificacion de actividad comercial", ZendeskDependencia.Gobernacion, 0, 49, 9, 1, 1, 10, 1);

        var consejeria = AddCategory(10, "Consejeria Juridica");
        Add(consejeria, "Atencion a llanta averiada por bache", ZendeskDependencia.ConsejeriaJuridica, 0, 51, 10, 1, 1, 11, 1);

        var c2 = AddCategory(11, "C2");
        Add(c2, "Atencion a emergencias (policia)", ZendeskDependencia.C2, 0, 52, 11, 1, 1, 12, 1);
        Add(c2, "Atencion a accidente vehicular", ZendeskDependencia.C2, 0, 53, 10, 1, 1, 11, 1);

        var callCenter = AddCategory(9, "Call Center");
        Add(callCenter, "Llamada", ZendeskDependencia.CallCenter);
        Add(callCenter, "Informativo", ZendeskDependencia.CallCenter, 0, 26, 7, 1, 1, 8, 1);
